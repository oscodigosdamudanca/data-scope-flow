-- Reordered migration to satisfy dependencies
...
-- User roles policies
create policy if not exists "UserRoles: select self or developer" on public.user_roles
for select to authenticated
using (user_id = auth.uid() or public.has_role(auth.uid(), 'developer'));

create policy if not exists "UserRoles: manage only developer" on public.user_roles
...
create policy if not exists "Profiles: select self or developer" on public.profiles
for select to authenticated
...
create policy if not exists "Profiles: update self or developer" on public.profiles
for update to authenticated
using (auth.uid() = id or public.has_role(auth.uid(), 'developer'))
with check (auth.uid() = id or public.has_role(auth.uid(), 'developer'));

create policy if not exists "Profiles: delete only developer" on public.profiles
...
create policy if not exists "Companies: select member or developer" on public.companies
for select to authenticated
using (
...
create policy if not exists "Companies: insert self owner or developer" on public.companies
for insert to authenticated
with check (
  created_by = auth.uid() or public.has_role(auth.uid(), 'developer')
);

create policy if not exists "Companies: update admin or developer" on public.companies
for update to authenticated
using (
  public.has_role(auth.uid(), 'developer')
...
create policy if not exists "Companies: delete only developer" on public.companies
for delete to authenticated
using (public.has_role(auth.uid(), 'developer'));

-- Membership policies
create policy if not exists "Memberships: select member or developer" on public.company_memberships
for select to authenticated
using (
  public.has_role(auth.uid(), 'developer')
...
create policy if not exists "Memberships: insert admin or developer" on public.company_memberships
for insert to authenticated
with check (
  public.has_role(auth.uid(), 'developer')
...
create policy if not exists "Memberships: update admin or developer" on public.company_memberships
for update to authenticated
using (
  public.has_role(auth.uid(), 'developer')
  or public.is_company_admin(auth.uid(), company_id)
...
create policy if not exists "Memberships: delete admin or developer" on public.company_memberships
for delete to authenticated
using (
  public.has_role(auth.uid(), 'developer')
  or public.is_company_admin(auth.uid(), company_id)
...
create trigger if not exists trg_companies_add_creator_membership
